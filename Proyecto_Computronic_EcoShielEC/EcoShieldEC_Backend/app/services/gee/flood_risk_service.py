...
class FloodRiskService:
    ...
    def get_environmental_features(
        self, sentinel_image, dem_image, point
    ) -> EnvironmentalFeatures:
        ...
        elevation_value = self.dem_service.get_elevation_value_at_point(
            dem_image, point
        )
    ...
    def build_analysis(self, features: EnvironmentalFeatures) -> AnalysisModel:
        ...
        if score <= 1:
            ...
            risk_level = "MEDIUM"
            recommended_action = (
                "Review the area and consider preventive mitigation actions."
            )
        else:
            risk_level = "HIGH"
            recommended_action = (
                "Prioritize field inspection and flood preparedness planning."
            )
        ...
        if reasons:
            ...
            message = (
                "No significant flood risk indicators were detected at this point."
            )
    ...
    def analyze_location(
        ...
        region = self.geometry_service.create_buffered_point(lat, lng, 20000)

        sentinel_image = self.collection_service.get_collection(
            region, start_date, end_date, cloud_cover_percentage
        )

        dem_image = self.collection_service.get_collection_elevation(region=region)

        features = self.get_environmental_features(sentinel_image, dem_image, point)
